 ...
 *
 * Disassembly of DSDT.aml, Mon May 29 09:42:15 2017
 *
...
DefinitionBlock ("", "DSDT", 2, "LENOVO", "CB-01   ", 0x00000001)
{
     ...
     * disassembly, but only 19 were resolved (7 unresolved). Additional
    ...
    Scope (_SB.PCI0.LPCB)
    {
        Device (EC0)
        {
                ...
                Method (_BST, 0, Serialized)  // _BST: Battery Status
                {
                    ...
                    If (LEqual (ECON, One))
                    {
                        If (LEqual (Acquire (LFCM, 0xA000), Zero))
                        {
                            ...
                            Store (B1RC, OBRC)
                            Store (B1FV, OBPV)
                            While (LOr (LNotEqual (B1RC, OBRC), LNotEqual (B1FV, OBPV)))
                            {
                                Store (B1RC, OBRC)
                                Store (B1FV, OBPV)
                            }
            ...
            Scope (\)
            {
                OperationRegion (LFCN, SystemMemory, 0x75F50D98, 0x00E7)
